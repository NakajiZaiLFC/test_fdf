...
bool	fdf_draw(t_fdf *fdf)
{
	...
	while (y < fdf->map.height)
	{
		...
		{
			// iso_projectionの結果を正しく格納
			p0 = iso_projection(x, y, fdf->map.points[y][x].z);
			// 横方向の線を描画
			if (x < fdf->map.width - 1)
			{
				p1 = iso_projection(x + 1, y, fdf->map.points[y][x + 1].z);
			...
			{
				p1 = iso_projection(x, y + 1, fdf->map.points[y + 1][x].z);
				draw_line_to_image(&fdf->img, p0, p1,
					fdf->map.points[y][x].color);
			}
			x++;
